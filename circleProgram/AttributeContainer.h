...
class AttributeContainer {
 public:
  ...
  //Objects of this clasß should not be copied accidentally.
  ...
  //If we want to make a copy, then we can add a method for it.
  ...
  size_t getNumberOfVertices() const { //Returns the number of vertices added to the container.
    if(attributeTypes.empty() ) {
      return 0;
    }
    else {
      return attributeTypes[0].counter;
    }
  }

  void printData() { 
    //I only used this function to check whether the code was correct. 
 ...
 private:
  ...
  void sendAttributesToGPU() {
    ...
    //Notice that the amount of storage reserved on the GPU is the number determined by the
    //number of vertices given to the reserve() function and not the number of vertices.
